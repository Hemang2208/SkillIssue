...
export default function SkillDetailPage() {
    ...
    async function handleDelete() {
        setWorking(true)
        try { await deleteSkill(skill.id); navigate(-1) }
            ...
            <main className="relative min-h-screen pt-28 pb-24">
                ...
                <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {/* Download */}
                        <button onClick={handleDownload} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-accent text-navy font-satoshi font-bold text-sm hover:bg-[#6bbcff] hover:shadow-[0_0_20px_rgba(75,169,255,0.3)] transition-all duration-300">
                        ...
                        {/* Guest CTA */}
                        {isGuest && (
                            <button onClick={signIn} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-accent/20 bg-accent/[0.06] text-accent font-satoshi font-semibold text-sm hover:bg-accent/15 hover:border-accent/40 transition-all duration-300 ml-auto">
                        ...
                        {/* Logged in Save CTA */}
                        {!isGuest && !isOwner && (
                            <button onClick={handleSaveSkill} disabled={saving} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl border font-satoshi font-semibold text-sm transition-all duration-300 ml-auto ${isSaved ? 'border-accent/40 bg-accent/20 text-accent' : 'border-accent/20 bg-accent/[0.06] text-accent hover:bg-accent/15 hover:border-accent/40'} disabled:opacity-50`}>
                                <svg className={`w-4 h-4 ${saving ? 'animate-spin' : ''}`} fill={isSaved ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isSaved ? 0 : 1.5}>
                                    {saving ? (
